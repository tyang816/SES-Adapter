# example
sbatch --export=dataset=DeepLocMulti,pooling_method=attention1d --job-name=deeploc-1_binary_mean_ef script/slurm/train_wofsss.slurm
...
for dataset in DeepSol DeepSoluE
do
    for plm_model in esm2_t36_3B_UR50D ankh-base ankh-large
    do
...
do
    sbatch --export=dataset=DeepLocBinary,pooling_method=$pooling_method --job-name=deeploc-1_binary_"$pooling_method"_ef script/slurm/train.slurm
    sbatch --export=dataset=DeepLocBinary,pooling_method=$pooling_method --job-name=deeploc-1_binary_"$pooling_method"_ef script/slurm/train_wofsss.slurm
    sbatch --export=dataset=DeepLocBinary,pooling_method=$pooling_method --job-name=deeploc-1_binary_"$pooling_method"_ef script/slurm/train_wofs.slurm
    sbatch --export=dataset=DeepLocBinary,pooling_method=$pooling_method --job-name=deeploc-1_binary_"$pooling_method"_ef script/slurm/train_woss.slurm
done
...
do
    ...
    do
        sbatch --export=dataset=DeepLocBinary,num_heads=$head_num,pooling_dropout=$pooling_dropout --job-name=deeploc-1_binary_attention1d_"$head_num"_"$pooling_dropout"_ef script/slurm/train_head.slurm
        sbatch --export=dataset=DeepLocMulti,num_heads=$head_num,pooling_dropout=$pooling_dropout --job-name=deeploc-1_multi_attention1d_"$head_num"_"$pooling_dropout"_ef script/slurm/train_head.slurm
